<?php 
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT r.*,CONCAT(c.firstname,' ',c.middlename,'' ,c.lastname) as client from `repair_list` r inner join client_list c on r.client_id = c.id where r.id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k)){
                        ...
                        <div class="row">
                            <div class="col-12">
                                <table class="table table-bordered border-info">
                                    ...
                                    <tr>
                                        <th class="text-muted text-white bg-gradient-dark px-2 py-1">Assigned Technician</th>
                                        <td><?= isset($tech_name) ? $tech_name : '' ?></td>
